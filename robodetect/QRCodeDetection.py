...
def distance(p, q):
    return math.sqrt(
        math.pow(math.fabs(p[0] - q[0]), 2) + math.pow(math.fabs(p[1] - q[1]),
                                                       2))
...
def getVertices(contours, cid, slope, quad):
    ...
    if (slope > 5 or slope < -5):
        ...
        halfy = (A[1] + D[1]) / 2
        for i in range(len(contours[cid])):
            if (contours[cid][i][0][0] < halfx and contours[cid][i][0][
                1] <= halfy):
                dmax[2], M0 = updateCorner(contours[cid][i][0], C, dmax[2], M0)
            elif (contours[cid][i][0][0] >= halfx and contours[cid][i][0][
                1] < halfy):
...
def find_qr_code_on_image(image):
    img = image
    # show the image
    ...
    if mark > 2:
        ...
        if (top < len(contours) and right < len(contours) and bottom < len(
                contours) and cv2.contourArea(
            contours[top]) > 10 and cv2.contourArea(
            contours[right]) > 10 and cv2.contourArea(
            contours[bottom]) > 10):
            tempL = []
            tempM = []
            tempO = []
            ...
            warped = cv2.warpPerspective(img, warp_matrix, (400, 400))
            ...
            code_image = Image.fromarray(warped)
            codes = zbarlight.scan_codes('qrcode', code_image)
            print('codes {}'.format(codes))
            if codes:
                return codes[0], img, warped
            else:
                return None
...
if __name__ == '__main__':
    ...
    while stream.isOpened():
        ...
        img = image
        ret = find_qr_code_on_image(image)
        if ret is not None:
            code, img, warped = ret[0], ret[1], ret[2]
            cv2.imshow('warped', warped)
